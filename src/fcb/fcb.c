...
int fcb_flash_read(const struct fcb *fcbp, const uint32_t sector, off_t off,
				   void *dst, size_t len)
{
	int rc;

	if (off + len > fcbp->f_sector_size)
	{
		return -EINVAL;
	}

	rc = fil_read(fcbp->offset + sector + off, dst, len);
...
int fcb_flash_write(const struct fcb *fcbp, const uint32_t sector, off_t off,
					const void *src, size_t len)
{
	...
	rc = fil_write(fcbp->offset + sector + off, src, len);
...
int fcb_erase_sector(const struct fcb *fcbp, const uint32_t sector)
{
	int rc;

	rc = fil_erase(fcbp->offset + sector, fcbp->f_sector_size);
	if (rc != 0)
	{
		return -EIO;
	}
...
int fcb_init(struct fcb *fcbp)
{
	...
	uint32_t oldest_sector = -1, newest_sector = -1;
	...
	if (fcbp->f_sector_cnt - fcbp->f_scratch_cnt < 1)
	{
		return -EINVAL;
	}

	fparam = fil_get_flash_parameters();
	fcbp->f_erase_value = fparam->erase_value;

	align = fparam->write_block_size;
	if (align == 0U)
	{
		return -EINVAL;
	}
 ...
 */
int fcb_sector_hdr_init(struct fcb *fcbp, uint32_t sector, uint16_t id)
{
	struct fcb_disk_area fda;
	int rc;
 ...
 */
int fcb_sector_hdr_read(struct fcb *fcbp, uint32_t sector, struct fcb_disk_area *fdap)
{
	struct fcb_disk_area fda;
	int rc;
