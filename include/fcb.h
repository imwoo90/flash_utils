...
#define FCB_FLAGS_CRC_DISABLED BIT(0)
     ...
     */
    int fcb_walk(struct fcb *fcbp, uint32_t sector_offset, fcb_walk_cb cb, void *cb_arg);
     ...
     */
    int fcb_flash_read(const struct fcb *fcbp, const uint32_t sector_offset, off_t off,
                       void *dst, size_t len);
     ...
     * @return  0 on success, negative errno code on fail.
     */
    int fcb_flash_write(const struct fcb *fcbp, const uint32_t sector_offset, off_t off,
